...
class FilePickerBuilder {
    ...
    private fun start(fragment: Fragment, requestCode: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(fragment.context!!, FilePickerConst.PERMISSIONS_FILE_PICKER) != PackageManager.PERMISSION_GRANTED) {
                Toast.makeText(
        ...
        PickerManager.providerAuthorities = fragment.context!!.applicationContext.packageName + ".droidninja.filepicker.provider"
