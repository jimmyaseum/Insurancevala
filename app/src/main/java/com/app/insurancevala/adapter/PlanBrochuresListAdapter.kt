...
class PlanBrochuresListAdapter(
    private val context: Context?,
    private val arrayList: ArrayList<PlanBrochuresModel>,
    private val recyclerItemClickListener: RecyclerClickListener
) : RecyclerView.Adapter<PlanBrochuresListAdapter.ViewHolder>() {
        ...
        fun bindItems(
            ...
            if (arrayList[position].UpdatedBy == 0 || arrayList[position].UpdatedBy == null) {
                if (!arrayList[position].CreatedOn.isNullOrEmpty()) {
                    ...
                    itemView.txtCreatedBy.text =
                        "Created By " + arrayList[position].CreatedByName + " On " + mdate + " " + mtime
                }

            } else {
                if (!arrayList[position].UpdatedOn.isNullOrEmpty()) {
                    val mdate = convertDateStringToString(
                        arrayList[position].UpdatedOn!!,
                        AppConstant.ddMMyyyy_HHmmss,
                        AppConstant.dd_LLL_yyyy
                    )
                    val mtime = convertDateStringToString(
                    ...
                    itemView.txtCreatedBy.text =
                        "Updated By " + arrayList[position].UpdatedByName + " On " + mdate + " " + mtime
                }
            }
            ...
            itemView.imgAttachment.setOnClickListener {
