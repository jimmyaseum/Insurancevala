...
import kotlinx.android.synthetic.main.adapter_add_closing_amount.view.*

class AddClosingAmountAdapter(
    val arrayList: ArrayList<ClosingAmountInfoModel>?,
    val inquiryTypeModel: ArrayList<InquiryTypeModel>,
    val recyclerItemClickListener: RecyclerItemClickListener
) : RecyclerView.Adapter<AddClosingAmountAdapter.ViewHolder>() {
    var context: Context? = null
    var views: View? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context)
            .inflate(R.layout.adapter_add_closing_amount, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        arrayList[position].tilClosingAmount = holder.itemView.edtClosingAmount
    }
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItems(
            context: Context,
            position: Int,
            arrayList: ArrayList<ClosingAmountInfoModel>?,
            arrayListInquiryType: ArrayList<InquiryTypeModel>?,
            recyclerItemClickListener: RecyclerItemClickListener
        ) {

            LogUtil.d(TAG, "" + arrayList!![position].ClosingAmount)

            if (arrayList[position].ClosingAmount!!.trim() != "0.00") {
                itemView.edtClosingAmount.setText(arrayList[position].ClosingAmount)
            }

            itemView.txtInquiryType.text = arrayListInquiryType!![position].InquiryType


            itemView.edtClosingAmount.setSimpleListener {
                arrayList[adapterPosition].ClosingAmount = it.toString()
                itemView.edtClosingAmount.setError(null)
            }
        }
    ...
    fun isValidateItem(): Boolean {
        ...
        if (!arrayList.isNullOrEmpty()) {
            for (model in arrayList) {
                if (model.tilClosingAmount != null) {
                    if (TextUtils.isEmpty(model.tilClosingAmount?.text!!.trim())) {
                        model.tilClosingAmount?.setError(
    ...
    private fun hideErrorTIL(position: Int) {
        if (!arrayList.isNullOrEmpty()) {
            for (i in arrayList.indices) {
                arrayList[i].tilClosingAmount?.setError(null)
            }
        }
    }
