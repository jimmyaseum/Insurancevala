...
class InquirySubTypeMultiSelectAdapter(
    val recyclerItemClickListener: RecyclerClickListener,
    val arrayList: ArrayList<InquirySubTypeModel>?
) : RecyclerView.Adapter<InquirySubTypeMultiSelectAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindItems(
            model: InquirySubTypeModel,
            recyclerItemClickListener: RecyclerClickListener,
            position: Int
        ) {
            itemView.cbName.text = model.InquirySubType
    ...
    fun getCheckBoxSelected(): List<String>? {
        ...
        for (i in arrayList!!.indices) {
            if (arrayList[i].IsSelected!!) {
                arrayListInquiryType.add(arrayList[i].InquirySubType!!)
            }
        }
